...
const reconstructed=predictionComparison({latest_residual:{v_mps:.01,omega_radps:-.1},model_visualization:{observations:[{left:.5,right:1,v_mps:.04,omega_radps:.2}]}});
assert.deepStrictEqual(reconstructed.command,{left:.5,right:1});
assert(Math.abs(reconstructed.expected.forward_mps-.03)<1e-12);
assert(Math.abs(reconstructed.expected.turn_radps-.3)<1e-12);
