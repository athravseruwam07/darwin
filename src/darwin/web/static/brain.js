...
foot.append(source);
if(thought.channel==='darwin'){const speak=document.createElement('button');speak.className='thought-speak';speak.type='button';speak.textContent='▶';speak.title='Replay this thought';speak.hidden=thought.voice!=='ready'&&!(canSpeakLocally&&thought.voice==='failed');speak.onclick=()=>playThought(thought.thought_id,true);foot.append(speak);}
li.append(foot);
return li;}
...
if(existing.voice!==thought.voice){existing.voice=thought.voice;const speak=existing.node.querySelector('.thought-speak');if(speak)speak.hidden=thought.voice!=='ready'&&!(canSpeakLocally&&thought.voice==='failed');}}
...
function playThought(id,manual){if(!voiceOn&&!manual)return;const thought=thoughts.find(item=>item.thought_id===id);if(!thought)return;played.add(id);speaking=true;rail.dataset.status='speaking';pick('brain-status').textContent='Speaking';if(thought.voice==='failed'&&speakLocally(thought))return;audio.src='/api/brain/voice/'+encodeURIComponent(id);audio.play().catch(()=>{if(speakLocally(thought))return;speaking=false;if(!manual){voiceOn=false;storage.set('darwin-brain-voice','off');applyVoiceButton();}});}
...
if(voiceOn){const ready=speakable(thoughts,played,canSpeakLocally);for(const id of ready)if(!queue.includes(id))queue.push(id);pump();}else{for(const thought of thoughts)played.add(thought.thought_id);queue=[];}
...
if(voiceOn){played=new Set(thoughts.slice(0,-1).map(thought=>thought.thought_id));queue=speakable(thoughts,played,canSpeakLocally);pump();}else{audio.pause();if(canSpeakLocally)window.speechSynthesis.cancel();speaking=false;queue=[];}
pollBrain();};
